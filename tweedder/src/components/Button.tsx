...
export default function Button({ small = false, gray = false, className = "", ...props }: ButtonProps) {
  ...
  return (
    <button className={`rounded-full transition-colors duration-200 disabled:cursor-not-allowed disabled:opacity-50 text-white ${colorClasses} ${sizeClasses} ${className}`} {...props}></button>
  )
}