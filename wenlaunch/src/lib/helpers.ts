...
export function formatReleaseDate(date: string) {
  ...
  const formattedReleaseDate = datetime && new Intl.DateTimeFormat("en-GB", {
    weekday: "short",
    day: "numeric",
    month: "numeric",
    year: "numeric",
  ...
  const formattedReleaseTime = datetime && new Intl.DateTimeFormat("en-GB", {
    timeZone: "UTC",
    timeZoneName: "short",
    hour: "numeric",
    minute: "numeric"
...
export function truncateBlockchainAddress(fullStr: string) {
  ...
  return (
    fullStr.substring(0, 7) +
    separator +
    fullStr.substring(fullStr.length - 7)
  );
...
export function getYesterdayDateString(format?: string) {
  return dayjs().subtract(1, 'days').utc().hour(0).startOf('hour').format(format);
}

export const allowedImageMIMETypes = ['image/png', 'image/jpg', 'image/jpeg', 'image/gif', 'image/svg', 'image/tiff'];
